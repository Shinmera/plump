...
(defun clone-attributes (node)
  (let ((map (make-attribute-map)))
    (loop for key being the hash-keys of (attributes node) using (hash-value val)
...
(defgeneric clone-node (node &optional deep)
  ...
  (:method ((table hash-table) &optional (deep T))
    ...
    (loop with map = (make-attribute-map)
          for key being the hash-keys of table using (hash-value val)
          do (setf (gethash key map) val)
          finally (return map)))
...
(defun previous-element (child)
  (let ((pos (1- (child-position child)))
    ...
    (loop while (< pos (fill-pointer family))
          do (let ((current (elt family pos)))
               (decf pos)
               (when (element-p current)
                 (return-from previous-element current))))))
  ...
  (let ((pos (1+ (child-position child)))
    ...
    (loop while (< pos (fill-pointer family))
          do (let ((current (elt family pos)))
               (incf pos)
               (when (element-p current)
                 (return-from next-element current))))))
...
(defgeneric serialize-object (node)
  ...
  (:method ((node xml-header))
    ...
    (let ((attributes (attributes node)))
      (wrs " " "version" "=\"")
      (%encode-entities (gethash "version" attributes) *stream* T)
      (wrs "\"")
      (loop for key being the hash-keys of attributes using (hash-value val)
            unless (string-equal key "version")
            do (wrs " " key)
               (when val
                 (wrs "=\"")
                 (%encode-entities val *stream* T)
                 (wrs "\""))))
    (wrs "?>"))
  (:method ((node cdata))
    (wrs "<![CDATA[" (text node) "]]>"))
  ...
  (:method ((table hash-table))
    (loop for key being the hash-keys of table using (hash-value val)
          do (wrs " " key)
             (when val
               (wrs "=\"")
               (%encode-entities val *stream* T)
